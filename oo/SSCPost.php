...
class SSCPost{
	...
	public function scribd_post( $key, $secret, $content, $title = "" ){
		...
		$file_name = $this->ssc->convert_to_pdf( $content, $title );
		if( count( $file_name ) ){
	...
	public function post_to_wordpresss( $link,  $title, $description, $tags = NULL, $categories = NULL, $queue_id = NULL  ){
	   if( strlen( $description ) > 1 ){
		...
		$postos = $this->ssc->sscdb->fetch_all_users( $token );
		...
		if( $count ){			
			...
			$response = $this->wordpress_post( 
					$this->ssc->sscdb->decrypt($posto['access_token']), 
					$this->ssc->sscdb->decrypt($posto['access_token_secret']),
			...
			return $this->ssc->sscdb->insert_queue_data($token);
		}
	  }
	}
	public function post_to_facebooks( $image, $link,  $title, $description, $snippet, $queue_id = NULL  ){
	   if( strlen( $description ) > 1 ){
		...
		if( $count ){			
			$index = rand(0, $count - 1 );
			$posto = $postos[$index];
			$user_id = $this->ssc->sscdb->decrypt( $posto['access_token_secret'] );
			$access_token = $this->ssc->sscdb->decrypt( $posto['access_token'] );
			$action_links = array(
				'name' => 'Share Post',
	   ...
	   if( strlen( $description ) > 1 ){
		...
		if( $count ){			
			...
			$response = $this->friendfeed_post( 
					trim($posto['username'] ), 
					$this->ssc->sscdb->decrypt( $posto['password'] ), 
	...
	public function post_to_pinterests( $link, $title, $image, $description, $queue_id = NULL  ){
	   if( strlen( $description ) > 1 ){
		...
		$count = count( $postos );
		if( $count ){			
			$posto = $this->ssc->sscdb->fetch_authority_user($token);
			if( !count( $posto ) ){
				$index = rand(0, $count - 1 );
				$posto = $postos[$index];
			}
		...
		if( $count ){			
			...
			if( $response != false ) {
				...
				if( count( $postos ) ){
					...
					$deliciouss = $this->ssc->array_random( $postos, $num );
					if( count( $deliciouss ) ){
	   ...
	   if( strlen($description ) > 1 ){
		...
		if( $count ){
			...
			if( is_array( $response ) ) {
				...
				if( count( $postos ) ){
					$num = rand( 0, count( $postos ) );
					$linkedins = $this->ssc->array_random( $postos, $num );
					if( count( $linkedins ) ){
	...
	public function process_twitter_queue( $id = NULL ){
		if( isset( $id ) ){
			$queue = $this->ssc->sscdb->fetch_twitter_item_by_id( $id  );
		} else {
			$queue = $this->ssc->sscdb->fetch_twitter_item( );
		}
		if( count( $queue ) ){
			$this->twitter_follower( $queue[0]['username'] );
			$this->ssc->sscdb->insert_twitter_data( array( 'id' => $queue[0]['id'], 
									'is_follower' => 1,
									'status' => 1 ) );	
		}
	}
	public function process_queue( $id = NULL ){
		if( isset( $id ) ){
			$queue = $this->ssc->sscdb->fetch_queue_item_by_id( $id  );
		} else {
			$queue = $this->ssc->sscdb->fetch_queue_item( );
		}
		if( count( $queue ) ){
			switch ( $queue[0]['social'] ){
				...
				case 'delicious':
					if( $queue[0]['action'] == 'bookmark' ){
						...
						$account = $this->ssc->sscdb->fetch_account_data( $queue[0]['social_id'] );
						$response = $this->delicious_post( 
								trim($account[0]['username'] ), 
								$this->ssc->sscdb->decrypt( $account[0]['password']), 
								$queue[0]['title'],  
								$queue[0]['link'], 
								$queue[0]['description'] );
						...
						$this->ssc->sscdb->insert_queue_data($token);
					}
				...
				case 'wordpress':
					if( $queue[0]['action'] == 'post' ){
						...
						if( isset( $queue[0]['social_id'] ) ){
							...
							$this->ssc->sscdb->decrypt($account[0]['access_token']),
				...
				case 'tumblr':
					if( $queue[0]['action'] == 'post' ){
						...
						if( isset( $queue[0]['social_id'] ) ){
							...
							$this->ssc->sscdb->decrypt($account[0]['access_token']), 
							$this->ssc->sscdb->decrypt($account[0]['access_token_secret']),
							trim( $account[0]['username'] ), 
			...
			if( !count( $this->ssc->sscdb->check_for_posting( $queue[0]['post_id'] ) )){
				$queue = $this->ssc->sscdb->fetch_scribd_queue( $queue[0]['post_id'] );
				if( count( $queue  )){
					$links = $this->ssc->sscdb->check_for_posting( $queue[0]['post_id'], 1 );
