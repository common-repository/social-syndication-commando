...
class SSCAccount extends WP_List_Table {
	...
	public function column_social($item) {
		if( $item['authority'] == 1 ){
			$actions = array(
			'delete'    => sprintf('<a href="?page=%s&sscaccountdelete=%s">Delete</a>',$_REQUEST['page'],$item['id']),
			'authority'    => sprintf('<a href="?page=%s&sscauthority=%s&sscval=0&sscsocial=%s">Remove Authority</a>',
								$_REQUEST['page'],$item['id'],$item['social'] ),
			);
		} else {
			$actions = array(
			'delete'    => sprintf('<a href="?page=%s&sscaccountdelete=%s">Delete</a>',$_REQUEST['page'],$item['id']),
			'authority'    => sprintf('<a href="?page=%s&sscauthority=%s&sscval=1&sscsocial=%s">Make Authority</a>',
								$_REQUEST['page'],$item['id'],$item['social'] ),
			);
		}
