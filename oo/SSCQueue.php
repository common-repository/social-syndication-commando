...
class SSCQueue extends WP_List_Table {
	...
	public function column_default( $item, $column_name ){
		//var_dump( $item );
        	switch( $column_name ){
					...
					case 'post_time':
						return  $ssc->time_stamp( $item['posted_timestamp'] );
		            		default:
		                		return '';
        	}
    	}
	...
	public function column_site($item) {
		if( ( $item['social'] == 'scribd' ) or ( $item['status'] == 1 ) ){
			$actions = array(
			'delete'    => sprintf('<a href="?page=%s&sscdeletequeue=%s">Delete</a>',$_REQUEST['page'],$item['id']),
			);
		} else {
			$actions = array(
			'edit'      => sprintf('<a href="?page=%s&sscpostnow=%s">Post Now</a>',$_REQUEST['page'],$item['id']),
			'delete'    => sprintf('<a href="?page=%s&sscdeletequeue=%s">Unqueue</a>',$_REQUEST['page'],$item['id']),
			);
		}
	...
	public function prepare_items(){
		...
		$data = $this->data;
        	...
        	function usort_reorder($a,$b){
            		$orderby = (!empty($_REQUEST['orderby'])) ? $_REQUEST['orderby'] : 'social';
